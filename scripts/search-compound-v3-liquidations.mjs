...
function selectChainProfile() {
  const key = String(process.env.COMP_LIQ_CHAIN ?? 'ethereum').toLowerCase();
...
async function rpc(rpcUrl, method, params, retries = 2) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    const timer = setTimeout(() => controller.abort(), envNumber('COMP_LIQ_RPC_TIMEOUT_MS', 20_000));
    try {
...
async function main() {
  ...
  const latestBlock = Number(process.env.COMP_LIQ_TO_BLOCK ?? hexToNumber(await rpc(rpcUrl, 'eth_blockNumber', [])));
  const lookbackBlocks = Math.floor(envNumber('COMP_LIQ_LOOKBACK_BLOCKS', 50_000));
  const fromBlock = Math.max(1, latestBlock - lookbackBlocks);
  const accountLimit = Math.floor(envNumber('COMP_LIQ_ACCOUNT_LIMIT', 200));
  const minNetProfitUsd = envNumber('COMP_LIQ_MIN_NET_PROFIT_USD', 5);
  const minReturnPct = envNumber('COMP_LIQ_MIN_RETURN_ON_BASE_PCT', 0.1);
  const gasUnits = Math.floor(envNumber('COMP_LIQ_GAS_UNITS', 1_200_000));

  console.error(`[compound-liq] chain=${CHAIN.name} market=${CHAIN.market} latest=${latestBlock} lookback=${lookbackBlocks}`);
  const base = await loadCometAssets(rpcUrl, latestBlock);
  ...
  const nativeUsd = envNumber('COMP_LIQ_ETH_USD', tokenPrices[base.assets.find((a) => a.symbol === 'WETH')?.asset?.toLowerCase()] ?? 1760);
  const gasPriceWei = BigInt(await rpc(rpcUrl, 'eth_gasPrice', []));
  const gasUsd = (Number(gasPriceWei) * gasUnits * nativeUsd) / 1e18;
  const baseAmounts = String(process.env.COMP_LIQ_BASE_AMOUNTS ?? '10,100,1000')
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean)
  ...
  const reserves = decodeIntWord(await ethCall(rpcUrl, CHAIN.comet, SELECTORS.getReserves, latestBlock), 0);
  const targetReserves = decodeUintWord(await ethCall(rpcUrl, CHAIN.comet, SELECTORS.targetReserves, latestBlock), 0);
  const reservesBelowTarget = reserves < BigInt(targetReserves);

  const accounts = await collectRecentAccounts(rpcUrl, fromBlock, latestBlock, accountLimit);
  console.error(`[compound-liq] accounts=${accounts.size} assets=${base.assets.length}`);
  ...
  for (const entry of accounts.values()) {
    ...
    let account;
    try {
      account = await readAccount(rpcUrl, entry.address, base.assets, latestBlock);
    } catch (err) {
      candidates.push({
        id: `compound-v3-liq-${CHAIN.shortName}-${entry.address.slice(2, 8).toLowerCase()}-error`,
        chain: CHAIN.name,
    ...
    if (account.borrowBalance === 0n && !account.collaterals.length) continue;
    const best = account.isLiquidatable
      ? await estimateBestBuy(rpcUrl, account, base, tokenPrices, baseAmounts, gasUsd, latestBlock)
      : null;
    const gate = gateFor(account, best, reservesBelowTarget, minNetProfitUsd, minReturnPct);
    candidates.push({
      ...
      observedFromCometEvents: [...entry.eventTopicsSeen],
      firstSeenBlock: entry.firstSeenBlock,
      account: {
        isLiquidatable: account.isLiquidatable,
        borrowBalance: account.borrowBalance.toString(),
        borrowBalanceHuman: formatBaseUnits(account.borrowBalance, base.baseDecimals),
        collateralCount: account.collaterals.length,
      },
      collaterals: account.collaterals.slice(0, 8),
      bestEstimate: best,
      gate: {
        ...gate,
        minNetProfitUsd,
      ...
      liveInterface: {
        status: gate.status === 'pass' ? 'compound-liquidation-plan-ready-needs-fork-simulation' : 'blocked-by-compound-liquidation-gate',
        requiresCex: false,
        userFlow: 'connect wallet, optionally flash-borrow base asset, call Comet absorb, then buyCollateral for discounted collateral when reserves are below target',
        requiredContracts: {
          comet: CHAIN.comet,
        },
        selectors: {
        ...
        productionStatus: 'not-enabled-until-compound-liquidation-adapter-and-fork-simulation-pass',
      },
    });
    if (checked % 25 === 0) console.error(`[compound-liq] checked=${checked} candidates=${candidates.length}`);
  }
  ...
  const artifact = {
    ...
    methodology: {
      classification: 'pure-on-chain-compound-v3-liquidation-arbitrage-scan',
      isPureArbitrage: true,
      noCexRequired: true,
      lookbackBlocks,
      fromBlock,
      toBlock: latestBlock,
      accountDiscovery: 'recent Supply, Withdraw, and Transfer events on the Compound V3 Comet market',
      accountLimit,
      ...
      thresholds: {
        isLiquidatable: true,
        reservesBelowTarget: true,
        minNetProfitUsd,
        minReturnOnBasePct: minReturnPct,
      },
      caveats: [
        'This is a current-state liquidation scan, not a long-horizon APY backtest.',
        'The scanner requires current base reserves below target before treating buyCollateral as executable.',
        'Some real opportunities may appear only after absorb changes reserves; those are deliberately not passed here.',
        'A passing candidate must be simulated on a fork immediately before execution.',
        'Production execution requires a Comet liquidation adapter, flash-loan or prefunded base asset handling, and collateral unwind logic.',
      ],
    },
    summary: {
  ...
  const outJson = resolve(dataDir, `compound-v3-liquidation-candidates-${CHAIN.shortName}.json`);
  await writeFile(outJson, `${JSON.stringify(artifact, null, 2)}\n`);
  console.log(
    `compoundV3Liquidations=${artifact.summary.status} accounts=${accounts.size} candidates=${candidates.length} passing=${passing.length} artifact=${outJson}`,
  );
  for (const candidate of candidates.slice(0, 10)) {
    console.log(
      `${candidate.id} gate=${candidate.gate.status} liquidatable=${candidate.account?.isLiquidatable ?? 'n/a'} netUsd=${candidate.bestEstimate?.netProfitUsd?.toFixed?.(4) ?? 'n/a'} reason=${candidate.gate.reason}`,
    );
  }
}
