...
async fn main() -> Result<()> {
    ...
    println!("rpc: {}  token0={:?} token1={:?}  feeA={} feeB={} (total {:.1} bps)",
             mask(&cfg.rpc), cfg.t0, cfg.t1, cfg.fee_a, cfg.fee_b, fee_total_bps);
    println!("blocks: {} .. {} step {}  capitals: {:?}", cfg.from, cfg.to, cfg.step, cfg.capitals);
    println!();

    // Collect blocks.
    let mut blocks = Vec::new();
    let mut b = cfg.from;
    while b <= cfg.to { blocks.push(b); b += cfg.step; }
    ...
    for block in blocks {
        ...
        let t0 = cfg.t0; let t1 = cfg.t1;
        let fa = cfg.fee_a; let fb = cfg.fee_b;
    ...
    let mut results: Vec<(u128, Vec<(u64, f64)>)> = cfg.capitals.iter().map(|&c| (c, Vec::new())).collect();
    let mut errors = 0u64;
    for h in handles {
    ...
    println!();
    println!("=== results (exact Quoter quotes, net = gross - {:.1} bps pool fees) ===", fee_total_bps);
    println!("(NOTE: gas + bribe NOT yet subtracted; add them for realistic net)");
    println!();
    for (cap, mut edges) in results {
        edges.sort_by_key(|(b, _)| *b);
        let n = edges.len();
        if n == 0 { println!("capital {} (raw): no successful samples", cap); continue; }
        ...
        println!("  samples: {}  errors counted separately", n);
        println!("  mean net edge: {:.2} bps", sum_net / n as f64);
        println!("  max net edge:  {:.2} bps", max_net);
        println!("  profitable (net>0): {} / {} ({:.2}%)", profitable, n, profitable as f64 / n as f64 * 100.0);
        if profitable > 0 {
            ...
            println!("      trades/year: {:.0}  avg net/trade: {:.2} bps (${:.6})", trades_per_year, avg_net, usd_per_trade);
            println!("      ANNUALIZED: {:.2}% on ${:.4}  (BEFORE gas+bribe, NOT guaranteed)", annualized_pct, cap_human);
        }
        println!();
    }
    println!("DISCLAIMER: exact pool quotes only. Realistic execution subtracts gas (~${:.4}/tx),", cfg.gas_cost_usd());
...
async fn scan_block(
    ...
    for &cap in capitals {
        ...
        let weth = quote(provider, quoter, t0, t1, cap, fee_a, bid).await
            .with_context(|| format!("leg1 @ {}", block))?;
        // WETH -> USDC in pool B (fee_b)
        let usdc_back = quote(provider, quoter, t1, t0, weth, fee_b, bid).await
            .with_context(|| format!("leg2 @ {}", block))?;
        // gross edge bps = (back/cap - 1) * 10000
        let edge = if cap == 0 { 0.0 } else {
...
async fn quote(
    ...
    data.extend_from_slice(&[0u8; 12]); data.extend_from_slice(&token_in.into_array());
    data.extend_from_slice(&[0u8; 12]); data.extend_from_slice(&token_out.into_array());
    data.extend_from_slice(&AlloyU256::from(amount).to_be_bytes::<32>());
    let mut fw = [0u8; 32];
    fw[29] = (fee >> 16) as u8; fw[30] = (fee >> 8) as u8; fw[31] = fee as u8;
    data.extend_from_slice(&fw);
    data.extend_from_slice(&[0u8; 32]);
    let tx = TransactionRequest::default().to(quoter).input(Bytes::from(data).into());
    let res: Bytes = provider.call(&tx).block(block).await
        .map_err(|e| anyhow::anyhow!("quoter call: {}", e))?;
    let bytes = res.as_ref();
    if bytes.len() < 32 { anyhow::bail!("short"); }
...
impl Config {
    fn gas_cost_usd(&self) -> f64 { 0.05 } // approx polygon gas in USD
}

fn block_seconds(rpc: &str) -> f64 {
    if rpc.contains("arb-mainnet") { 0.25 }
    else if rpc.contains("base-mainnet") || rpc.contains("opt-mainnet") { 2.0 }
    else if rpc.contains("polygon") { 2.0 }
    else if rpc.contains("bnb") { 3.0 }
    else { 12.0 }
}

fn mask(u: &str) -> String {
    if let Some(idx) = u.find("/v2/") { format!("{}***", &u[..idx+4]) } else { u.to_string() }
}
...
fn parse_args() -> Result<Config> {
    ...
    let mut i = 1;
    let next = |i: &mut usize| -> Result<String> { let v = a.get(*i+1).cloned().ok_or_else(|| anyhow::anyhow!("missing"))?; *i += 2; Ok(v) };
    while i < a.len() {
        match a[i].as_str() {
            "--rpc" => rpc = next(&mut i)?,
    ...
    let capitals: Vec<u128> = capitals_s.split(',').filter_map(|s| s.trim().parse().ok()).collect();
    Ok(Config { rpc, t0: t0s.parse()?, t1: t1s.parse()?, fee_a, fee_b, capitals, from, to, step, concurrency })
}
