...
impl Triangle {
    fn fee_ab(&self) -> u32 { if self.fee_ab != 0 { self.fee_ab } else { self.fee } }
    fn fee_bc(&self) -> u32 { if self.fee_bc != 0 { self.fee_bc } else { self.fee } }
    fn fee_ca(&self) -> u32 { if self.fee_ca != 0 { self.fee_ca } else { self.fee } }
...
async fn main() -> Result<()> {
    ...
    println!("triangle: {:?} -> {:?} -> {:?} -> {:?}", cfg.t.a, cfg.t.b, cfg.t.c, cfg.t.a);
    println!("fee: {} bps/leg, capital: {} (raw)", cfg.t.fee / 100, cfg.capital);
    println!("blocks: {} .. {} step {} (concurrency {})", cfg.from, cfg.to, cfg.step, cfg.concurrency);
    println!();
    ...
    for &(block, edge) in &edges {
        ...
        if cfg.verbose || net > 0.0 {
            println!("  block {:>10}: gross {:8.2} bps, net {:8.2} bps {}",
                     block, edge, net, if net > 0.0 { "*** PROFITABLE" } else { "" });
        }
    }
    ...
    println!("total fee: {:.1} bps (legs {}/{}/{})", total_fee_bps, cfg.t.fee_ab(), cfg.t.fee_bc(), cfg.t.fee_ca());
    if n > 0 {
        ...
        if profitable > 0 {
            ...
            println!();
            println!("IF we captured every profitable block with ${:.0} each:", cfg.capital_human);
            println!("  trades/year: {:.0}", trades_per_year);
            println!("  avg net/trade: {:.2} bps", avg_net_bps);
            println!("  annualized return: {:.2}%  (CAPITAL {}, NOT guaranteed)", annualized, cfg.capital_human);
        }
    }
    println!();
...
async fn scan_block(
    ...
    // leg1: a->b
    let out1 = quote(provider, quoter, t.a, t.b, capital, t.fee_ab(), bid).await
        .with_context(|| format!("leg1 a->b fee={} @ {}", t.fee_ab(), block))?;
    // leg2: b->c
    let out2 = quote(provider, quoter, t.b, t.c, out1, t.fee_bc(), bid).await
        .with_context(|| format!("leg2 b->c fee={} @ {}", t.fee_bc(), block))?;
    // leg3: c->a
    let out3 = quote(provider, quoter, t.c, t.a, out2, t.fee_ca(), bid).await
        .with_context(|| format!("leg3 c->a fee={} @ {}", t.fee_ca(), block))?;
    // edge in bps = (out3/capital - 1) * 10000
    if capital == 0 { return Ok(0.0); }
...
async fn quote(
    ...
    if bytes.len() < 32 { anyhow::bail!("quoter returned short: {} bytes", bytes.len()); }
...
impl Triangle {
    fn clone(&self) -> Self {
        Triangle { a: self.a, b: self.b, c: self.c, fee: self.fee,
                   fee_ab: self.fee_ab, fee_bc: self.fee_bc, fee_ca: self.fee_ca }
    }
}

fn block_seconds(rpc: &str) -> f64 {
    if rpc.contains("arb-mainnet") { 0.25 }
    else if rpc.contains("opt-mainnet") { 2.0 }
    else if rpc.contains("base-mainnet") { 2.0 }
    else if rpc.contains("polygon-mainnet") { 2.0 }
    else if rpc.contains("bnb-mainnet") { 3.0 }
    else { 12.0 } // ethereum
}
...
fn parse_args() -> Result<Config> {
    ...
    while i < args.len() {
        let v = args[i].as_str();
        let next = || -> Result<String> { args.get(i+1).cloned().ok_or_else(|| anyhow::anyhow!("missing value for {}", v)) };
        match v {
            "--rpc" => { rpc = next()?; i += 2; }
            "--quoter" => { quoter = next()?; i += 2; }
            "--token-a" => { ta = next()?; i += 2; }
            "--token-b" => { tb = next()?; i += 2; }
            "--token-c" => { tc = next()?; i += 2; }
            "--fee" => { fee = next()?.parse()?; i += 2; }
            "--fee-ab" => { fee_ab = next()?.parse()?; i += 2; }
            "--fee-bc" => { fee_bc = next()?.parse()?; i += 2; }
            "--fee-ca" => { fee_ca = next()?.parse()?; i += 2; }
            "--capital" => { capital = next()?.parse()?; i += 2; }
            "--capital-human" => { capital_human = next()?.parse()?; i += 2; }
            "--from" => { from = next()?.parse()?; i += 2; }
            "--to" => { to = next()?.parse()?; i += 2; }
            "--step" => { step = next()?.parse()?; i += 2; }
            "--concurrency" => { concurrency = next()?.parse()?; i += 2; }
    ...
    Ok(Config { rpc, quoter, t: Triangle { a, b, c, fee, fee_ab, fee_bc, fee_ca }, capital, capital_human, from, to, step, concurrency, verbose })
}
