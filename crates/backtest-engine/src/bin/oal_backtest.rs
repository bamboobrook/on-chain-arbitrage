...
async fn main() -> Result<()> {
    ...
    println!("blocks: {} .. {} step {} ({} samples)",
             args.from, args.to, args.step, (args.to - args.from) / args.step);
    println!("capital: {} USDC base units (6 dec)", args.capital);
    println!("costs:   bribe {:.0}% of gross, gas {} @ {:.4} gwei",
             args.bribe_pct * 100.0, args.gas_units, args.gas_price_gwei);
    println!();
    ...
    while block <= args.to {
        let (sa, _ta, la) = match src.v3_state_at(&pool_a, block).await {
            Ok(s) => s,
            Err(e) => { eprintln!("  block {}: poolA read failed: {}", block, e); block += args.step; continue; }
        };
        let (sb, _tb, lb) = match src.v3_state_at(&pool_b, block).await {
            Ok(s) => s,
            Err(e) => { eprintln!("  block {}: poolB read failed: {}", block, e); block += args.step; continue; }
        };
        if la.is_zero() || lb.is_zero() { block += args.step; continue; }
        samples += 1;
        ...
        if sa.is_zero() || sb.is_zero() { block += args.step; continue; }
        let (larger, smaller, a_is_cheap) = if sa >= sb { (sa, sb, true) } else { (sb, sa, false) };
        ...
        if edge_bps > max_edge_bps { max_edge_bps = edge_bps; }
        ...
        if samples % 20 == 0 || gross_after_slip > 0.0 {
            println!("  block {:>10} edge {:6.1}bps dir {} gross ${:+.4} slip {:.1}% net ${:+.4}",
                     block, edge_bps, direction, gross_after_slip, slippage*100.0, net);
        }
        block += args.step;
    }
    ...
    println!("opportunities (>0): {} ({:.1}%)", opportunities, pct(opportunities, samples));
    println!("profitable (net>0): {} ({:.1}%)", profitable, pct(profitable, samples));
    ...
    println!();
    println!("annualized return:  {:.2}%  (capital ${:.0}, window {:.2} days)", annualized, capital_usdc, days);
    println!();
    println!("DISCLAIMER:");
...
fn fee_bps_of(pool: &str) -> u32 {
    ...
    if p.contains("c6962004") { 5 }
    else if p.contains("c473e2ae") { 30 }
    else { 30 } // default assume 0.30%
}
...
fn pct(x: u64, total: u64) -> f64 {
    if total == 0 { 0.0 } else { x as f64 / total as f64 * 100.0 }
}
...
fn parse_args() -> Result<Args> {
    let rpc = std::env::var("RPC_ARBITRUM_URL")
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--rpc" => { i += 1; if i < args.len() { /* override rpc */ let _ = &args[i]; } i += 0; }
            "--pool-a" => { i += 1; if i < args.len() { /* override */ } }
            _ => {}
        }
        i += 1;
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--rpc" => { rpc_v = args.get(i+1).cloned().unwrap_or(rpc_v); i += 2; }
            "--pool-a" => { pa = args.get(i+1).cloned().unwrap_or(pa); i += 2; }
            "--pool-b" => { pb = args.get(i+1).cloned().unwrap_or(pb); i += 2; }
            "--from" => { from = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(from); i += 2; }
            "--to" => { to = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(to); i += 2; }
            "--step" => { step = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(step); i += 2; }
            "--capital" => { capital_s = args.get(i+1).cloned().unwrap_or(capital_s); i += 2; }
            "--bribe-pct" => { bribe_pct = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(bribe_pct); i += 2; }
            "--gas-units" => { gas_units = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(gas_units); i += 2; }
            "--gas-price-gwei" => { gas_price_gwei = args.get(i+1).and_then(|s| s.parse().ok()).unwrap_or(gas_price_gwei); i += 2; }
            _ => { i += 1; }
        }
    }
    ...
    Ok(Args { rpc: rpc_v, pool_a: pa, pool_b: pb, from, to, step, capital, bribe_pct, gas_units, gas_price_gwei })
}
